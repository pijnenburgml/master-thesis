...
NDVI_100b100 <- resample(NDVI, Shannon_map, method="bilinear")
plot(NDVI_100b100)
NDVI_100b100_masked <- mask(NDVI_100b100, Shannon_map)
plot(NDVI_100b100_masked)
NDVI_100b100_masked_model <- mask(NDVI_100b100_masked, tile_DEM_aggregated_masked)
...
Sentinel_lattice <- readOGR("~/data/output/model_object.shp")
Sentinel_lattice$NDVI <- unlist(as.data.frame(NDVI_100b100_masked_model)[1])
...
Mod_Lattice_no_ndvi <- inla(formula,     
                    family = "gaussian", # have to change the family, not gaussian, try gamma
                    data = Sentinel_data,
                    control.compute = list(cpo = T, dic = T, waic = T,return.marginals.predictor=TRUE), verbose=TRUE, scale=1)
...
summary(Mod_Lattice)
plot(Sentinel_data$Shannon_10, Mod_Lattice$summary.fitted.values$mean, main="Fitting result")
observed <- Sentinel_data$Shannon_10
library(INLAutils)
plot_inla_residuals(Mod_Lattice, observed=observed)
residual_plot
